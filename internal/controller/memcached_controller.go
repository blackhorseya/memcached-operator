...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if memcached.Status.Conditions != nil || len(memcached.Status.Conditions) == 0 {
		...
		// Fetch the Memcached instance again to get the updated status
	...
	// Let's add a finalizer. Then, we can define some operations which should occur
	// before the custom resource is deleted. More info:
	// https://kubernetes.io/docs/concepts/overview/working-with-objects/finalizers
	if !controllerutil.ContainsFinalizer(memcached, memcachedFinalizer) {
		logger.Info("Adding Finalizer for the Memcached")
		ok := controllerutil.AddFinalizer(memcached, memcachedFinalizer)
		if !ok {
			logger.Error(err, "Failed to add finalizer to Memcached")
			return ctrl.Result{}, err
		}

		err = r.Update(ctx, memcached)
		if err != nil {
			logger.Error(err, "Failed to update Memcached with finalizer")
			return ctrl.Result{}, err
		}
	}

	// Check if the Memcached instance is marked to be deleted, which is indicated
	...
	if isMemcachedMarkedToBeDeleted {
		if controllerutil.ContainsFinalizer(memcached, memcachedFinalizer) {
			...
			// Perform all operations required before removing the finalizer and allow the
			// Kubernetes API to remove the custom resource.
			r.doFinalizerOperationsForMemcached(memcached)

			// TODO(user): If you add operations to the doFinalizerOperationsForMemcached
			// method then you need to ensure that all worked fine before deleting and
			// updating the Downgrade status otherwise, you should requeue here.

			// Re-fetch the memcached Custom Resource before updating the status so that we
			// have the latest state of the resource on the cluster and we will avoid
			// raising the error "the object has been modified, please apply your changes to
			...
			logger.Info("Removing Finalizer for Memcached after successfully perform the operations")
			ok := controllerutil.RemoveFinalizer(memcached, memcachedFinalizer)
			if !ok {
				logger.Error(err, "Failed to remove finalizer from Memcached")
				return ctrl.Result{}, err
			}
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			logger.Error(err, "Failed to create a new Deployment")
		...
		// Deployment created successfully We will requeue the reconciliation so that we
		// can ensure the state and move forward for the next operations
	...
	// to set the quantity of Deployment instances to the desired state on the
	// cluster. Therefore, the following code will ensure the Deployment size is the
	// same as defined via the Size spec of the Custom Resource which we are
	// reconciling.
...
func labelsForMemcached(_ string) map[string]string {
	var imageTag string
